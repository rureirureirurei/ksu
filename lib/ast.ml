...
let rec string_of_expr_aux offset {value; _} = match value with
  ...
  | Lambda { ids; body } ->
      let indent = String.make offset ' ' in
      "(lambda (" ^ String.concat " " ids ^ ")\n" ^ indent ^ "  " ^ string_of_expr_aux (offset + 2) body ^ ")"
  | If { cond; y; n } ->
      let indent = String.make offset ' ' in
      "(if " ^ string_of_expr_aux offset cond ^ "\n" ^ indent ^ "    " ^ string_of_expr_aux (offset + 4) y ^ "\n" ^ indent ^ "    " ^ string_of_expr_aux (offset + 4) n ^ ")"
  | Callcc e -> 
      "(callcc " ^ string_of_expr_aux (offset + 2) e ^ ")"
  | Let { defs; body } ->
      let indent = String.make offset ' ' in
      let defs_str = List.map (fun (name, expr) -> name ^ " " ^ string_of_expr_aux (offset + 3) expr) defs in
      "(let ("
      ...
      ^ ")\n" ^ indent ^ "  " ^ string_of_expr_aux (offset + 2) body ^ ")"
  | Pair (e1, e2) ->
      "(cons " ^ string_of_expr_aux offset e1 ^ " :: " ^ string_of_expr_aux offset e2 ^ ")"
  | Nil -> "nil"

let string_of_expr expr = string_of_expr_aux 0 expr

let string_of_top_expr {value; _} = match value with
  | Expr e -> string_of_expr e
  | Define { name; expr } ->
      "(define " ^ name ^ "\n  " ^ string_of_expr_aux 2 expr ^ ")"
